  ...
  <div class="login">
    ...
    <div v-if="type === 'login'" class="login login-body">
      <van-form>
        ...
        <div style="margin: 16px">
          <div class="link-register" @click="changeType('register')">
            立即注册
          </div>
          <van-button
            round
    ...
    <div v-else class="login-body register">
      <van-form>
        ...
        <div style="margin: 16px">
          <div class="link-register" @click="changeType('login')">
            已有登录账号
          </div>
          <van-button
            round
...
export default {
  setup() {
    ...
    const verifyRef = ref(null); // ref(null)获取dom结构
    ...
    const onSubmit = async () => {
      console.log(verifyRef.value.imgCode); // 通过ref.value 可以取到组件内setup函数的返回值
      if (
      ...
      if (state.type == "login") {
        ...
        localStorage.setItem("token", data); //本地存储
